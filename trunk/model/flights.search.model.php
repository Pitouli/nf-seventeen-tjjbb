	...
	if(isset($_POST['DdepartSearch'], $_POST['DarriveeSearch']))
	{
			...
			&& preg_match("#^([0-9]{2})/([0-9]{2})/([0-9]{4})$#", trim($_POST['DarriveeSearch']), $pregEnd))
		{
			...
			if(checkdate((int)$pregStart[2], (int)$pregStart[1], (int)$pregStart[0]) && checkdate((int)$pregEnd[2], (int)$pregEnd[1], (int)$pregEnd[0]))
			{
				...
				$resultVol = $bdd->query("
				SELECT v.id AS id, to_char(v.depart, 'DD/MM/YYYY HH24:MI') AS date_depart, to_char(v.arrive, 'DD/MM/YYYY HH24:MI') AS date_arrive, td.nom AS terminal_depart, ad.nom AS aeorport_depart, ta.nom AS terminal_arrive, aa.nom AS aeroport_arrive, m.nom AS avion, av.id AS n_avion
